...
FRAME_DIR = os.path.join(DATA_PATH, 'frames')  # Absolute path for extracted frames
FRAME_SIZE = (299, 299)
MAX_FRAMES = 30
BATCH_SIZE = 16
EPOCHS = 5
...
def extract_method_videos(data_path, dataset):
    ...
    for video in tqdm(os.listdir(videos_path)):
        if not video.endswith(('.mp4', '.avi', '.mov')):  # skip non-video files
            continue
...
if __name__ == '__main__':
    # Step 1: Extract frames (original only for now)
    extract_method_videos(DATA_PATH, 'original')     # Real YouTube videos
    # extract_method_videos(DATA_PATH, 'Deepfakes')  # Uncomment when Deepfakes added

    # Step 2: Train model
    model = train_model()

    # Step 3: Predict a test image (adjust to an actual extracted frame)
    test_image = os.path.join(FRAME_DIR, 'real', 'srw4r3htm4U', 'frame_000.jpg')  # Replace 'srw4r3htm4U' as needed
